...
public abstract class AlgoritmoGenetico {
	...
	public Individuo ejecuta() {
		...
		for (int i = 0; i < pasos; i++) {
			...
			recombinacion = recombinar(poblacion.individuo(a1).cromosoma(),poblacion.individuo(a2).cromosoma());
			recombinacion.mutar(probMutacion);
			Individuo nuevoInd = new Individuo(recombinacion,problema);
